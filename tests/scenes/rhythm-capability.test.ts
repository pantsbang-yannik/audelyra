...
import { VISUAL_TARGETS } from '../../src/scenes/nebula/mapping/types'
...
describe('律动能力矩阵（mixer v2：body → 消费的律动目标）', () => {
  ...
  it('每类至少消费一个目标，且全部落在 VISUAL_TARGETS 白名单内、无重复', () => {
    for (const targets of Object.values(RHYTHM_CAPABILITY)) {
      expect(targets.length).toBeGreaterThan(0)
      expect(new Set(targets).size).toBe(targets.length)
      for (const t of targets) expect(VISUAL_TARGETS).toContain(t)
    }
  })
